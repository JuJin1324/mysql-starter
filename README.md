...
> 특징 및 장점
> 1.순차적으로 처리  
> 2.바깥 테이블과 일치하는 값을 안쪽 테이블에서 찾아야 하므로 테이블의 해당 열에 인덱스 필요  
> 3.메모리 사용량은 가장 적음  
> 4.선행 테이블의 처리 범위가 전체 일의 양을 결정  
> 5.좁은 범위에서 유리한 성능을 보여줌  
> 6.순차적으로 처리하며, Random Access 위주  
> 7.후행 테이블에는 조인을 위한 인덱스 생성 필요  
> 8.실행속도 = 선행 테이블 사이즈 * 후행 테이블 접근 횟수  
> 
> 단점 및 주의사항  
> 1.데이터를 랜덤으로 엑세스하므로 결과 집합이 많으면 속도가 느려짐  
> 2.join index 가 없거나, 조인 집합을 구성하는 검색 조건이 조인 범위를 줄이지 못할 경우 비효율  
> 
