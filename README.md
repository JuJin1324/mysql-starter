...
> 그리고 JPA 의 경우 관계 설정을 위해서는 프라이머리 키를 통해서 관계를 설정하게됨으로 그로 인해 외래키의 크기도 기존 BIGINT(8) 에서 BINARY(16) 으로 2배 더 커지게 된다.  
> 또한 클러스터링 인덱스(PK)를 통한 조회 성능에서 이점을 보는 것 보다 INSERT/UPDATE/DELETE 에서 UUIDv4 를 사용하여 빈번한 순서 정렬로 인한 성능저하의 단점을 더 크게보는 시각있다.  
